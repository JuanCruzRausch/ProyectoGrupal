...
export function publicationSeller(seller_id) {
  return async (dispatch) => {
    return axios(
      "http://localhost:5050/publicationtest/sellerpublications/" + seller_id
    ).then((res) =>
      dispatch({ type: PUBLICATION_SELLER, payload: res.data.data })
    );
...
export function addPublication(id, data) {
  return async (dispatch) => {
    return axios
      .post(`http://localhost:5050/publicationtest/${id}`, data)
...
export function getAllProducts() {
  return async (dispatch) => {
    ...
    return axios(`http://localhost:5050/publicationtest?page=1`).then((res) =>
...
export function GetSingleProduct(id) {
  return async (dispatch) => {
    axios("http://localhost:5050/publicationtest/" + id).then((res) =>
...
export function getAllCategories() {
  const arr = [];
  return async (dispatch) => {
    return axios("http://localhost:5050/categories").then((res) => {
...
export function getPaginate(page) {
  return async (dispatch) => {
    return axios("http://localhost:5050/publicationtest?page" + page).then(
      (res) =>
        dispatch({
          type: GET_ALL_PRODUCTS,
          payload: res.data.data.publications,
        })
    );
  };
}
...
export function BuscarProducto(title, min, max) {
  return async (dispatch) => {
    dispatch({ type: SET_LOADING, payload: "spin" });
    return await axios(
      `http://localhost:5050/publicationtest/byName/${title}?price[lte]=${max}&price[gte]=${min}`
    ).then((res) => {
...
export function getProductByCategory(id, min, max) {
  return async (dispatch) => {
    dispatch({ type: SET_LOADING, payload: "spin" });
    return axios(
      `http://localhost:5050/publicationtest?category=${id}&price[lte]=${max}&price[gte]=${min}`
    ).then((res) => {
      dispatch({
...
export const deleteProduct = (id, userID, sellerId) => {
  ...
  return async (dispatch) => {
    return axios
      .post(`http://localhost:5050/seller/${id}/${userID}`)
      .then((res) => {
